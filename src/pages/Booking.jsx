...
const BookingPage = () => {
  ...
  // 1. Unique Tracking Code Generator
  ...
  // 2. Load Facilities dynamically from PHP API
  useEffect(() => {
    fetch(`${API_BASE}/get_facilities.php`)
      ...
      .then(data => {
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          ...
          const firstFacility = data.data[0];
          setFormData(prev => ({
            ...prev,
            facility_id: firstFacility.facility_id || firstFacility.id,
            venue_name: firstFacility.facility_name
          }));
        } else if (data.data) {
          setFacilities(data.data);
        }
      ...
      .catch(err => {
        ...
        showToast('Error', 'Failed to load amenities list.', 'error');
      })
  ...
  const isTimeInPast = () => {
    ...
    if (formData.reservation_date === todayStr) {
      ...
      return formData.start_time < currentTimeStr;
    }
    return false;
  };

  // Verify schedule availability with PHP backend
  const checkSlotAvailability = async () => {
    if (!formData.reservation_date || !formData.start_time || !formData.end_time) {
      showToast('Error', 'Please select a date and complete time frame.', 'error');
      return;
    }
    if (formData.start_time >= formData.end_time) {
      showToast('Time Error', 'End time must be later than start time.', 'error');
      return;
    }
    if (isTimeInPast()) {
      showToast('Time Error', 'Cannot select a time slot that has already passed today.', 'error');
      return;
    }

    setCheckingAvailability(true);
    const formattedTimeSlot = `${formData.start_time} - ${formData.end_time}`;
    ...
    try {
      ...
      if (data && !data.available) {
        showToast('Unavailable', 'This slot conflicts with an existing reservation.', 'error');
      } else {
        setCurrentStep(2);
      }
    } catch (err) {
      showToast('Error', 'Unable to verify schedule availability.', 'error');
    } finally {
  ...
  const validateStep = () => {
    ...
    if (currentStep === 1) {
      const basicFields = formData.reservation_date && formData.start_time && formData.end_time;
      return basicFields && (formData.start_time < formData.end_time) && !isTimeInPast();
    }
  ...
  const handleSubmit = async () => {
    ...
    data.append('facility_id', formData.facility_id);
    data.append('tracking_code', trackingCode);
    data.append('reservation_date', formData.reservation_date);
    ...
    try {
      ...
      if (result.success) {
        showToast('Success!', result.message || 'Reservation submitted successfully!', 'success');
        setTimeout(() => {
          setIsSubmitting(false);
          navigate('/services');
        }, 2500);
      } else {
        showToast('Error', result.message || 'Failed to complete booking.', 'error');
        setTrackingCode(generateTrackingCode());
        setIsSubmitting(false);
      }
    } catch (error) {
      showToast('Server Error', 'Invalid response from backend server.', 'error');
      setTrackingCode(generateTrackingCode());
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .ep-venue-card { border: 2px solid #cbd5e1; border-radius: 20px; padding: 30px 20px; text-align: center; cursor: pointer; background: rgba(255, 255, 255, 0.6); transition: all 0.3s ease; }
        ...
        .ep-venue-card i { font-size: 2.8rem; color: #94a3b8; }
        ...
        .ep-file-upload-box { border: 2px dashed #cbd5e1; border-radius: 16px; padding: 30px 20px; text-align: center; cursor: pointer; transition: 0.3s; background: rgba(255, 255, 255, 0.5); }
        ...
        .ep-upload-content h4 { margin: 10px 0 5px; color: #0f172a; font-size: 1rem; }
        .ep-upload-content p { margin: 0; color: #64748b; font-size: 0.8rem; }
        .ep-upload-icon { font-size: 2.5rem; color: #94a3b8; }
        .ep-preview-container { display: flex; flex-direction: column; align-items: center; gap: 4px; }
        .ep-preview-icon { font-size: 2rem; color: #059669; }
        .ep-file-name { font-weight: 700; font-size: 0.9rem; color: #0f172a; word-break: break-all; }
        .ep-file-change { font-size: 0.75rem; color: #059669; text-decoration: underline; }
        .ep-review-box { background: #ffffff; border-radius: 16px; padding: 25px; border: 1px solid #e2e8f0; }
        .ep-review-category { margin-top: 20px; }
        .ep-review-category h5 { color: #059669; font-size: 0.85rem; font-weight: 800; text-transform: uppercase; border-bottom: 1px solid #f1f5f9; padding-bottom: 8px; margin-bottom: 12px; }
        ...
        .ep-actions { display: flex; justify-content: space-between; margin-top: 40px; }
        ...
        .ep-btn-next:disabled { opacity: 0.5; cursor: not-allowed; box-shadow: none; }
        ...
        .slide-in { animation: fadeSlideUp 0.4s ease forwards; }
      ...
      <div className="ep-page-wrapper">
        <div className="ep-form-card">
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            {/* STEP 1: DYNAMIC FACILITY SELECTION */}
            {currentStep === 0 && (
              <div className="slide-in">
                <h4 className="ep-section-title"><i className="bi bi-building"></i> Select Available Facility</h4>
                {loadingFacilities ? (
                  <p style={{ textAlign: 'center', color: '#64748b', padding: '40px 0' }}>Loading available facilities from database...</p>
                ) : facilities.length === 0 ? (
                  <p style={{ textAlign: 'center', color: '#64748b', padding: '40px 0' }}>No facilities available for reservation at this time.</p>
                ) : (
                  <div className="ep-venue-grid">
                    {facilities.map((v) => {
                      const facId = v.facility_id || v.id;
                      const isSelected = String(formData.facility_id) === String(facId);
                      return (
                        <div 
                          key={facId} 
                          onClick={() => setFormData({ ...formData, facility_id: facId, venue_name: v.facility_name })} 
                          className={`ep-venue-card ${isSelected ? 'active' : ''}`}
                        >
                          <i className={`bi ${v.icon_class || 'bi-building'}`}></i>
                          <h4>{v.facility_name}</h4>
                          <p>{v.description || 'Barangay Resource / Facility'}</p>
                        </div>
                      );
                    })}
                  </div>
            ...
            {currentStep === 2 && (
              <div className="slide-in">
                ...
                <h4 className="ep-section-title" style={{ marginTop: '35px' }}><i className="bi bi-file-earmark-arrow-up"></i> Document Verification</h4>
                <div className="ep-grid">
                  {renderFilePreview('id_front', 'Valid ID (Front)', 'Upload a valid government-issued ID', 'bi-person-vcard')}
                  {renderFilePreview('id_holding', 'Verification Selfie', 'Selfie holding your valid ID clearly', 'bi-camera-fill')}
                </div>
              </div>
            )}
                ...
                <div className="ep-review-box">
                  ...
                  <div className="ep-review-category">
                    ...
                    <div className="ep-review-row">
                      ...
                      <span className="ep-review-val">{formData.venue_name}</span>
                    </div>
                    <div className="ep-review-row">
                      <span className="ep-review-label">Target Date</span>
                      <span className="ep-review-val">{formData.reservation_date}</span>
                    </div>
                    <div className="ep-review-row">
                      <span className="ep-review-label">Time Frame</span>
                      <span className="ep-review-val">{formData.start_time} to {formData.end_time}</span>
                    </div>
                  </div>
                  ...
                  <div className="ep-review-category">
                    ...
                    <div className="ep-review-row">
                      ...
                      <span className="ep-review-val">{formData.contact_name.toUpperCase()}</span>
                    </div>
                    <div className="ep-review-row">
                      <span className="ep-review-label">Contact Reference</span>
                      <span className="ep-review-val">{formData.contact_number}</span>
                    </div>
                    <div className="ep-review-row">
            ...
            <div className="ep-actions">
              <button 
                ...
                disabled={currentStep === 0 || checkingAvailability || isSubmitting} 
              ...
              <button 
                ...
                disabled={!validateStep() || checkingAvailability || isSubmitting} 
