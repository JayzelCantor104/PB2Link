...
import '../styles/auth1.css'; // Inherits existing crisp canvas/login form theme
...
const SetupPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api_backend/admin_setup_password.php', {
        method: 'POST',
      ...
      if (data.success) {
        setMessage({ type: 'success', text: 'Password configured successfully! Redirecting to login...' });
        // FIXED: Redirect to the unified /login route
  ...
  return (
    <main style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      <div className="auth-card text-start">
        ...
        {message.text && (
          <div
            className={message.type === 'success' ? 'success-banner' : 'error-banner'}
            style={{
              padding: '12px',
              borderRadius: '8px',
              marginBottom: '16px',
